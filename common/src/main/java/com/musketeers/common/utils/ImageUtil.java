...
public class ImageUtil {
     ...
     * 生成缩略图到指定的目录
     *
     * @param compressPath  目录
     * @param files 要生成缩略图的文件列表
     * @param  compressServerPath 返回的目录
     * @throws IOException
     */
    public static List<String> generateThumbnail2Directory(String compressPath,String compressServerPath, String... files) throws IOException {
        return generateThumbnail2Directory(DEFAULT_SCALE, compressPath,compressServerPath, files);
    }
     ...
     *
     * @param scale    图片缩放率
     * @param compressPath 缩略图保存目录
     * @param compressServerPath 返回的缩略图服务位置
     * @param files    要生成缩略图的文件列表
     * @throws IOException
     */
    public static List<String> generateThumbnail2Directory(double scale, String compressPath,String compressServerPath, String... files) throws IOException {
        Thumbnails.of(files)
                // 图片缩放率，不能和size()一起使用
                .scale(scale)
                // 缩略图保存目录,该目录需存在，否则报错
                .toFiles(new File(compressPath), Rename.SUFFIX_HYPHEN_THUMBNAIL);
        List<String> list = new ArrayList<>(files.length);
        for (String file : files) {
            list.add(appendSuffix(file,compressServerPath, SUFFIX));
        }
        return list;
    }
     ...
     * @return
     */
    public static String appendSuffix(String fileName,String compressServerPath, String suffix) {
        String newFileName = "";

        int indexOfDot = fileName.lastIndexOf('.');
        int indexOfI= fileName.lastIndexOf('/');

        if (indexOfDot != -1) {
            newFileName = fileName.substring(indexOfI, indexOfDot);
            newFileName += suffix;
        ...
        return compressServerPath+newFileName;
    }
